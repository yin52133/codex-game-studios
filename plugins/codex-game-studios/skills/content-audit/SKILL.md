---
name: content-audit
description: "Audit GDD-specified content counts against implemented content. Identifies what's planned vs built. (source alias: /content-audit)."
---
...
- Source workflow alias: `/content-audit`.
...
- Source argument hint: `[system-name | --summary | (no arg = full audit)]`.
...
2. **L0 pre-scan**: Before full-reading any GDDs, rg search all GDD files for
   `## Summary` sections plus common content-count keywords:
   ```
   rg search pattern="(## Summary|N enemies|N levels|N items|N abilities|enemy types|item types)" glob="design/gdd/*.md" output_mode="files_with_matches"
   ```
...
what has been implemented. Use file search and rg search to locate files.
...
- rg search for dialogue data files in `assets/data/`
