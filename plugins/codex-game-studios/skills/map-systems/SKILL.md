---
name: map-systems
description: "Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index. (source alias: /map-systems)."
---
...
- Source workflow alias: `/map-systems`.
...
- Source argument hint: `[next | system-name] [--review full|lean|solo]`.
...
Then use `request user input` to capture feedback:
...
Use `request user input` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via Codex sub-agent delegation using gate TD-SYSTEM-BOUNDARY (`references/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `request user input` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via Codex sub-agent delegation using gate PR-SCOPE (`references/docs/director-gates.md`) before writing the index.**
...
Using the template at `references/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Codex sub-agent delegation using gate CD-SYSTEMS (`references/docs/director-gates.md`).**
...
- Codex sub-agent delegation: Systems decomposition
...
Use `request user input` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `request user input`:
...
After the systems index is created (or after designing some systems), present next actions using `request user input`:
