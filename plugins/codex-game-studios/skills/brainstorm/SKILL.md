---
name: brainstorm
description: "Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration. (source alias: /brainstorm)."
---
...
- Source workflow alias: `/brainstorm`.
...
- Source argument hint: `[genre or theme hint, or 'open'] [--review full|lean|solo]`.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `request user input`** at key decision points throughout brainstorming:
   ...
   `request user input` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an request user input with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `request user input` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `request user input` with these exact tab labels:
...
Present all three. Then use `request user input` to capture the selection.
...
Ask these as `request user input` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `request user input`:
...
If the user selects B, C, or D, make the revision, then use `request user input` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via Codex sub-agent delegation in parallel before moving to Phase 5. Issue both Codex sub-agent delegation calls simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`references/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`references/docs/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `request user input`:
...
- **Target platform**: Use `request user input` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `request user input` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via Codex sub-agent delegation using gate TD-FEASIBILITY (`references/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Codex sub-agent delegation using gate PR-SCOPE (`references/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `references/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `request user input` for write approval:
...
If [B]: ask which section to revise using `request user input` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `request user input` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `references/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
