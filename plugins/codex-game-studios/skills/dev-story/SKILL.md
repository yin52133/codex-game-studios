...
- Source workflow alias: `/dev-story`.
...
- Source argument hint: `[story-path]`.
...
If they differ, use `request user input` before proceeding:
...
### Engine reference
Read `references/docs/technical-preferences.md`:
...
specialist to spawn via Codex sub-agent delegation.
...
Read the `Engine Specialists` section of `references/docs/technical-preferences.md`
...
Spawn the chosen programmer agent(s) via Codex sub-agent delegation with the full context package:

Brief the agent with file paths and targeted reading instructions — do not serialize document content into the Codex sub-agent delegation prompt. The agent reads what it needs directly:
...
5. **Engine preferences**: `references/docs/technical-preferences.md` — read naming conventions and performance budgets
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via Codex sub-agent delegation. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
