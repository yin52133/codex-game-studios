---
name: localize
description: "Full localization pipeline: scan for hardcoded strings, extract and manage string tables, validate translations, generate translator briefings, run cultural/sensitivity review, manage VO localization, test RTL/platform requirements, enforce string freeze, and report coverage. (source alias: /localize)."
---
...
- Source workflow alias: `/localize`.
...
- Source argument hint: `[scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa]`.
...
Spawn `localization-lead` via Codex sub-agent delegation. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
rg search `src/` for VO audio references. Verify each referenced path exists in `assets/audio/vo/[locale]/`. Report broken references.
...
Read `references/docs/technical-preferences.md` to determine the engine. Then check:
...
Spawn `localization-lead` via Codex sub-agent delegation with:
