...
- Source workflow alias: `/architecture-review`.
...
- Source argument hint: `[focus: full | coverage | consistency | engine | single-gdd path/to/gdd.md]`.
...
Before reading any full document, use rg search to extract `## Summary` sections
from all GDDs and ADRs:

```
rg search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 4
rg search pattern="## Summary" glob="docs/architecture/adr-*.md" output_mode="content" -A 3
```

For `single-gdd [path]` mode: use the target GDD's summary to identify which
ADRs reference the same system (rg search ADRs for the system name), then full-read
...
### Project Standards
- `references/docs/technical-preferences.md`
...
- rg search all ADRs for API names listed in `deprecated-apis.md`
...
After completing the engine audit above, spawn the **primary engine specialist** via Codex sub-agent delegation for a domain-expert second opinion:
...
If [B]: display the complete proposed systems-index section, then re-ask with `request user input`.
...
Use `request user input` for the write approval:
...
For `rtm` mode, use `request user input`:
...
Then close with `request user input` tailored to the pre-gate checklist state:
...
3. **Offer options** via request user input with three choices:
...
5. **Use `request user input` for write approvals** — plain text "May I?" is not
