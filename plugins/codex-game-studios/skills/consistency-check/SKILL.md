...
- Source workflow alias: `/consistency-check`.
...
- Source argument hint: `[full | since-last-review | entity:<name> | item:<name>]`.
...
## Phase 3: rg search-First Conflict Scan
...
```
rg search pattern="[entity_name]" glob="design/gdd/*.md" output_mode="content" -C 3
```
...
```
(Or use rg search with wider context if the file is large)
...
Then close with an `request user input` widget:
