---
name: team-level
description: "Orchestrate level design team: level-designer + narrative-director + world-builder + art-director + systems-designer + qa-tester for complete area/level creation. (source alias: /team-level)."
---
...
- Source workflow alias: `/team-level`.
...
- Source argument hint: `[level name or area to design] [--review full|lean|solo]`.

## Ported Workflow
When this skill is invoked:

**Decision Points:** At each step transition, use `request user input` to present
...
Use the Codex sub-agent delegation to spawn each team member as a subagent:
...
Spawn all three agents simultaneously — issue all three Codex sub-agent delegation calls before waiting for any result.
...
**Gate**: Use `request user input` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `request user input` with options:
...
**Gate**: Use `request user input` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `request user input` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `request user input` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all subagent outputs are collected, spawn `level-designer` via Codex sub-agent delegation to compile and write the final document:
...
to sub-agents spawned via Codex sub-agent delegation. Each sub-agent enforces the "May I write to [path]?"
...
If any spawned agent (via Codex sub-agent delegation) returns BLOCKED, errors, or cannot complete:

1. **Surface immediately**: Report "[AgentName]: BLOCKED — [reason]" to the user before continuing to dependent phases
2. **Assess dependencies**: Check whether the blocked agent's output is required by subsequent phases. If yes, do not proceed past that dependency point without user input.
3. **Offer options** via request user input with choices:
