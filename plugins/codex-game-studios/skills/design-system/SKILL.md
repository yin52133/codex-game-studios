---
name: design-system
description: "Guided, section-by-section GDD authoring for a single game system. Gathers context from existing docs, walks through each required section collaboratively, cross-references dependencies, and writes incrementally to file. (source alias: /design-system)."
---
...
- Source workflow alias: `/design-system`.
...
- Source argument hint: `<system-name> [--review full|lean|solo]`.
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `request user input`:
...
- Read `references/docs/technical-preferences.md` to identify the engine and version
...
Use the template structure from `references/templates/game-design-document.md`:
...
File content:
- Codex sub-agent delegation: Designing [system-name] GDD
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `request user input` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `request user input` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `request user input`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `request user input` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `request user input`:
...
spawn `creative-director` via Codex sub-agent delegation:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via Codex sub-agent delegation in parallel:
...
- Surface any disagreements between agents to the user via `request user input`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via Codex sub-agent delegation in parallel:
...
- Present the specialists' proposals to the user for review via `request user input`
...
For required systems: **spawn `art-director` via Codex sub-agent delegation** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via Codex sub-agent delegation using gate **CD-GDD-ALIGN** (`references/docs/director-gates.md`).
...
```
rg search pattern="  - name: [candidate_name]" path="design/registry/entities.yaml"
```
...
- Codex sub-agent delegation: [system-name] GDD
...
**When delegating via Codex sub-agent delegation**:
...
- The main session presents the agent's output to the user via `request user input`
- The user decides; the main session writes to file
...
2. **request user input** at every decision point (Explain -> Capture pattern):
