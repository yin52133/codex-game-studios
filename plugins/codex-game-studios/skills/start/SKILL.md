---
name: start
description: "First-time onboarding — asks where you are, then guides you to the right workflow. No assumptions. (source alias: /start)."
---
...
- Source argument hint: `[no arguments]`.
...
Check:
- **Engine configured?** Read `references/docs/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Use `request user input` with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not request user input (it's an open response).
2. Acknowledge the concept, then use `request user input` to offer two paths:
...
**If it does not exist**: Use `request user input`:
...
After presenting the recommended path, use `request user input` to ask the user which step they'd like to take first. Never auto-run the next skill.
