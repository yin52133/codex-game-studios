---
name: gate-check
description: "Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts. Use when user says 'are we ready to move to X', 'can we advance to production', 'check if we can start the next phase', 'pass the gate'. (source alias: /gate-check)."
---
...
- Source workflow alias: `/gate-check`.
...
- Source argument hint: `[target-phase: systems-design | technical-setup | pre-production | production | polish | release] [--review full|lean|solo]`.
...
5. **Production** — Feature development (Epic/Feature/Codex sub-agent delegation tracking active)
...
- [ ] Technical preferences configured (`references/docs/technical-preferences.md` populated)
...
- For localization checks: `rg search` for hardcoded strings in `src/`
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Codex sub-agent delegation using the parallel gate protocol from `references/docs/director-gates.md`. Issue all four Codex sub-agent delegation calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`references/docs/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`references/docs/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`references/docs/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`references/docs/director-gates.md`)
...
> **Tool-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (Read tool) or re-running a specific check (rg search tool) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `request user input`.
...
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `references/templates/difficulty-curve.md` — or use `/quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `references/templates/player-journey.md` — or author it collaboratively using `/ux-design` Phase 2b.
