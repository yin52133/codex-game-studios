---
name: create-epics
description: "Translate approved GDDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — run /create-stories [epic-slug] after each epic is created. (source alias: /create-epics)."
---
...
- Source workflow alias: `/create-epics`.
...
- Source argument hint: `[system-name | layer: foundation|core|feature|presentation | all] [--review full|lean|solo]`.
...
rg search all GDDs for their `## Summary` sections before reading anything fully:
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via Codex sub-agent delegation using gate **PR-EPIC** (`references/docs/director-gates.md`).
