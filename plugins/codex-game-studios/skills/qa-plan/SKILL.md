...
- Source argument hint: `[sprint | feature: system-name | story: path]`.
...
**Argument:** `$ARGUMENTS` (blank = ask user via request user input)
...
- **No argument** — use `request user input`:
...
**Engine**: [engine name from references/docs/technical-preferences.md, or "Not configured"]
...
then ask two questions together using `request user input`:
...
- Use `request user input` for scope selection when no argument is provided.
