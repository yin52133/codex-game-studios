...
- Source workflow alias: `/smoke-check`.
...
- Source argument hint: `[sprint | quick | --platform pc|console|mobile|all]`.
...
3. **Engine detection**: read `references/docs/technical-preferences.md` and
...
"Engine not configured in `references/docs/technical-preferences.md`. Run
...
Use `request user input` to batch-verify. Keep to at most 3 calls.
...
- Use `request user input` for all manual smoke check verification.
