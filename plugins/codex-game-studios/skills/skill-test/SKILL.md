---
name: skill-test
description: "Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report). (source alias: /skill-test)."
---
...
- Source workflow alias: `/skill-test`.
...
- Source argument hint: `static [skill-name | all] | spec [skill-name] | category [skill-name | all] | audit`.
...
- `static all` → run 7 structural checks on all skills (file search `skills/*/SKILL.md`)
...
**FAIL** if `tool guidance` includes `Write` or `Edit` but no ask-before-write language is found.
...
`invocation input hint` must be non-empty. If the skill body mentions multiple modes
...
file search `skills/*/SKILL.md` to get the complete list of skills.
