...
- Source argument hint: `[focus: full | gdds | adrs | stories | infra]`.
...
- `references/docs/technical-preferences.md` — engine configured?
...
If the project appears fresh (no artifacts at all), use `request user input`:
...
1. **Parenthetical status values** — rg search for any Status cell containing
...
Read `references/docs/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
and offer to handle it immediately using `request user input`. Choose the first
branch that applies:
