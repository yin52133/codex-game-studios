---
name: asset-spec
description: "Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after art bible and GDD/level design are approved, before production begins. (source alias: /asset-spec)."
---
...
- Source workflow alias: `/asset-spec`.
...
- Source argument hint: `[system:<name> | level:<name> | character:<name>] [--review full|lean|solo]`.

## Ported Workflow
If no argument is provided, check whether `design/assets/entity-inventory.md` exists:
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `request user input`:
...
Present the full proposed inventory to the user in conversation. Then use `request user input`:
...
- **Technical preferences**: Read `references/docs/technical-preferences.md` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `request user input`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `request user input`:
...
Present the full identified list to the user. Use `request user input`:
...
Spawn specialist agents based on review mode. **Issue all Codex sub-agent delegation calls simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** via Codex sub-agent delegation:
...
**`technical-artist`** via Codex sub-agent delegation:
...
After presenting all specs, use `request user input`:
...
```
rg search pattern="ASSET-" path="design/assets/asset-manifest.md"
```
