---
name: prototype
description: "Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after /brainstorm and /setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict. (source alias: /prototype)."
---
...
- Source workflow alias: `/prototype`.
...
- Source argument hint: `[concept-description] [--path html|engine|paper] [--review full|lean|solo] [--spike]`.
...
Otherwise, use `request user input` to confirm intent before proceeding:
...
Read `AGENTS.md` and `references/docs/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `request user input` with your
...
Read `references/templates/prototype-report.md` to get the report structure.
...
- `full` → spawn `creative-director` via Codex sub-agent delegation using gate **CD-PLAYTEST** if
...
2. **Choose path** — same request user input widget as Phase 3 (HTML / Engine / Paper).
