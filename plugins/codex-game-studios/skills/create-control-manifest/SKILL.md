...
- Source workflow alias: `/create-control-manifest`.
...
- Source argument hint: `[update — regenerate from current ADRs]`.
...
Spawn `technical-director` via Codex sub-agent delegation using gate **TD-MANIFEST** (`references/docs/director-gates.md`).
...
- **CONCERNS** → surface via `request user input` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
