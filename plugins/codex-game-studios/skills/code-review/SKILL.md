---
name: code-review
description: "Performs an architectural and quality code review on a specified file or set of files. Checks for coding standard compliance, architectural pattern adherence, SOLID principles, testability, and performance concerns. (source alias: /code-review)."
---
...
- Source workflow alias: `/code-review`.
...
- Source argument hint: `[path-to-file-or-directory]`.
...
Read `references/docs/technical-preferences.md`, section `## Engine Specialists`. Note:
...
Spawn all applicable specialists simultaneously via Codex sub-agent delegation — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` via Codex sub-agent delegation in parallel with the engine specialists. Pass:
