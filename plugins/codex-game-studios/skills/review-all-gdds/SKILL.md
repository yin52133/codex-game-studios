...
- Source workflow alias: `/review-all-gdds`.
...
- Source argument hint: `[focus: full | consistency | design-theory | since-last-review]`.
...
```
rg search pattern="## Summary" glob="design/gdd/*.md" output_mode="content" -A 5
```
...
the same GDD inputs but produce separate reports. Spawn both as parallel Codex sub-agent delegation
...
**When spawning parallel Codex sub-agent delegation agents for Phase 2 and Phase 3, always pass:**
...
- The engine name and version from `references/docs/technical-preferences.md` and `docs/engine-reference/[engine]/VERSION.md`

Do not rely on the subagent to re-read these files — it has its own context window and cannot access Phase 1 results unless they are explicitly passed in the Codex sub-agent delegation prompt.
...
Use `request user input` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `request user input`:
...
After all file writes are complete, use `request user input` for a closing widget.
