...
- Source workflow alias: `/create-architecture`.
...
- Source argument hint: `[focus-area: full | layers | data-flow | api-boundaries | adr-audit] [--review full|lean|solo]`.
...
3. `references/docs/technical-preferences.md` — naming conventions, performance budgets,
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `request user input`:
...
Apply gate **TD-ARCHITECTURE** (`references/docs/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` via Codex sub-agent delegation using gate LP-FEASIBILITY (`references/docs/director-gates.md`):**
...
Use `request user input` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `request user input`:
