---
name: story-done
description: "End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint. (source alias: /story-done)."
---
...
- Source workflow alias: `/story-done`.
...
- Source argument hint: `[story-file-path] [--review full|lean|solo]`.
...
3. If multiple in-progress stories are found, use `request user input`:
...
- **No hardcoded values check**: `rg search` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: `rg search` for player-facing strings in `src/`
  that should be in localization files.
- **Dependency check**: if a criterion says "depends on X", check that X exists.

### Manual verification with confirmation (use `request user input`)
...
Batch up to 4 manual verification questions into a single `request user input` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `file search` and `rg search`)
   - **Integration test**: check `tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified via `request user input`
...
- If found: read the file and check the sign-off table for unchecked boxes. rg search for lines matching `| .* | .* | .* | \[ \] Approved` (a sign-off row with an unchecked checkbox). If any unchecked sign-off rows are found: flag as **ADVISORY** — "Evidence file found at `[path]` but [N] sign-off(s) are still pending (shown as `[ ] Approved` in the sign-off table). Obtain required sign-offs before final closure. Note: for solo developers, all roles may be signed off by the same person."
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   `rg search` the implemented files for key function names, data structures, or class
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). `rg search` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: `rg search` the implemented files for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Codex sub-agent delegation using gate **QL-TEST-COVERAGE** (`references/docs/director-gates.md`).
...
- `lean` → use `request user input` before proceeding:
...
Spawn `lead-programmer` via Codex sub-agent delegation using gate **LP-CODE-REVIEW** (`references/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `request user input`:
...
Use `request user input` before writing anything:
...
- Use `request user input` for the code review prompt and for batching manual
