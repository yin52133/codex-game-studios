...
- Source workflow alias: `/story-readiness`.
...
- Source argument hint: `[story-file-path or 'all' or 'sprint']`.
...
See `references/docs/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via request user input)
...
If no argument is given, use `request user input`:
...
in conversation. Do not use file edit tools — the user (or
...
Spawn `qa-lead` via Codex sub-agent delegation using gate **QL-STORY-READY** (`references/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `request user input`:
