---
name: bug-report
description: "Creates a structured bug report from a description, or analyzes code to identify potential bugs. Ensures every bug report has full reproduction steps, severity assessment, and context. (source alias: /bug-report)."
---
...
- Source workflow alias: `/bug-report`.
...
- Source argument hint: `[description] | analyze [path-to-file]`.
...
2. **Search the codebase** for related files using rg search/file search to add context (affected system, likely files).
...
1. **Re-run reproduction steps** — use rg search/file search to check whether the root cause code path still exists as described. If the fix removed or changed it, note the change.
