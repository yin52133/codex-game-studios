---
name: skill-improve
description: "Improve a skill using a test-fix-retest loop. Runs static checks, proposes targeted fixes, rewrites the skill, re-tests, and keeps or reverts based on score change. (source alias: /skill-improve)."
---
...
- Source workflow alias: `/skill-improve`.
...
- Source argument hint: `[skill-name]`.
...
- **Check 4 fail** → file edit in tool guidance but no ask-before-write language
- **Check 5 warn** → no follow-up or next-step section at the end
- **Check 6 warn** → `context: fork` set but fewer than 5 phases found
- **Check 7 warn** → invocation input hint is empty or doesn't match documented modes
