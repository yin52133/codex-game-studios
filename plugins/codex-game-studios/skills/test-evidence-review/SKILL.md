...
- Source workflow alias: `/test-evidence-review`.
...
- Source argument hint: `[story-path | sprint | system-name]`.
...
**Logic stories**: file search `tests/unit/[system]/[story-slug]_test.*`
  - If not found, also try: rg search in `tests/unit/[system]/` for files
...
Heuristics:
- rg search test file for "zero", "max", "null", "empty", "min", "invalid",
