...
- Source workflow alias: `/test-flakiness`.
...
- Source argument hint: `[ci-log-path | scan | registry]`.
...
For Unreal projects: automation logs go to `Saved/Logs/`. rg search for
...
- rg search for `<testcase name=` to get test names
- rg search for `<failure` or `<error` to identify failures
- Parse `classname` and `name` attributes for full test identifiers

**Plain text logs**:
- rg search for pass/fail patterns:
...
Use rg search to check the test file for timing calls, randf, global state access,
