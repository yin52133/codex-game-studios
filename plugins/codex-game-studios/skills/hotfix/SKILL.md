---
name: hotfix
description: "Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly. (source alias: /hotfix)."
---
...
- Source argument hint: `[bug-id or description]`.
...
Confirm with `request user input`:
...
If the check passes, use `request user input` before creating the branch:
...
Use the Codex sub-agent delegation to request sign-off in parallel:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via Codex sub-agent delegation with:
- The hotfix description and affected system
- The regression test results from Phase 5
- A list of all systems that touch the changed files (use rg search to find callers)
