...
description: "Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate. (source alias: /regression-suite)."
---
...
- Source workflow alias: `/regression-suite`.
...
- Source argument hint: `[update | audit | report]`.
...
- `/regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `request user input`:
...
1. rg search `tests/unit/[system]/` and `tests/integration/[system]/` for file names
...
2. rg search `tests/unit/[system]/` and `tests/integration/[system]/` for a test
...
- GDD sections added or revised since the regression suite was last updated
  (use rg search on GDD file modification hints if available, or ask the user)
