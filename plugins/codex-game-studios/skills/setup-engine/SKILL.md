---
name: setup-engine
description: "Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data. (source alias: /setup-engine)."
---
...
- Source workflow alias: `/setup-engine`.
...
- Source argument hint: `[engine] | [engine version] | refresh | upgrade [old-version] [new-version] | no args for guided selection`.
...
**Question 1 — Prior experience** (ask this first, always, via `request user input`):
...
**Question 2 — Target platform** (ask this second, always, via `request user input` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `request user input` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `request user input` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `references/docs/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `request user input`:
...
- Use rg search to search for deprecated API names extracted from the migration
