---
name: sprint-plan
description: "Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs. (source alias: /sprint-plan)."
---
...
- Source workflow alias: `/sprint-plan`.
...
- Source argument hint: `[new|update|status] [--review full|lean|solo]`.
...
- If the file doesn't exist and this is a `new` sprint: use `request user input`:
...
| ID | Codex sub-agent delegation | Agent/Owner | Est. Days | Dependencies | Acceptance Criteria |
|----|------|-------------|-----------|-------------|-------------------|
...
| Codex sub-agent delegation | Reason | New Estimate |
|------|--------|-------------|
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `request user input` to gather changes.
...
### Completed
| Codex sub-agent delegation | Completed By | Notes |
|------|-------------|-------|

### In Progress
| Codex sub-agent delegation | Owner | % Done | Blockers |
|------|-------|--------|----------|

### Not Started
| Codex sub-agent delegation | Owner | At Risk? | Notes |
|------|-------|----------|-------|

### Blocked
| Codex sub-agent delegation | Blocker | Owner of Blocker | ETA |
|------|---------|-----------------|-----|
...
Before finalising the sprint plan, spawn `producer` via Codex sub-agent delegation using gate **PR-SPRINT** (`references/docs/director-gates.md`).
...
If CONCERNS, use `request user input`:
