---
name: security-audit
description: "Audit the game for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure, and input validation gaps. Produces a prioritised security report with remediation guidance. Run before any public release or multiplayer launch. (source alias: /security-audit)."
---
...
- Source workflow alias: `/security-audit`.
...
- Source argument hint: `[full | network | save | input | quick]`.
...
Read `references/docs/technical-preferences.md` to determine:
...
Spawn `security-engineer` via Codex sub-agent delegation. Pass:
...
rg search patterns: `File.open`, `load`, `deserialize`, `JSON.parse`, `from_json`, `read_file` — check each for validation.
...
rg search for: `recv`, `receive`, `PacketPeer`, `socket`, `NetworkedMultiplayerPeer`, `rpc`, `rpc_id` — check each call site for validation.
...
rg search for: `get_input`, `Input.get_`, `input_map`, user-facing text fields — check validation.
...
rg search for: `api_key`, `secret`, `password`, `token`, `private_key`, `DEBUG`, `print(` in release-facing code.
