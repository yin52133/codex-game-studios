...
description: "Generates a comprehensive milestone progress review including feature completeness, quality metrics, risk assessment, and go/no-go recommendation. Use at milestone checkpoints or when evaluating readiness for a milestone deadline. (source alias: /milestone-review)."
---
...
- Source workflow alias: `/milestone-review`.
...
- Source argument hint: `[milestone-name|current] [--review full|lean|solo]`.
...
Before generating the Go/No-Go recommendation, spawn `producer` via Codex sub-agent delegation using gate **PR-MILESTONE** (`references/docs/director-gates.md`).
...
If OFF TRACK, use `request user input` before generating the recommendation:
...
If AT RISK, use `request user input`:
