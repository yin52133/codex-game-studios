---
name: design-review
description: "Reviews a game design document for completeness, internal consistency, implementability, and adherence to project design standards. Run this before handing a design document to programmers. (source alias: /design-review)."
---
...
- Source workflow alias: `/design-review`.
...
- Source argument hint: `[path-to-design-doc] [--depth full|lean|solo]`.
...
| Engine-specific patterns or APIs | Primary engine specialist (from `references/docs/technical-preferences.md`) |
...
**CRITICAL: Codex sub-agent delegation in this skill spawns a SUBAGENT — a separate independent Codex sub-agent session
...
actual Codex sub-agent delegation calls. A simulated review is not a specialist review.**

Issue all Codex sub-agent delegation calls simultaneously. Do NOT spawn one at a time.
...
Use `request user input` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `request user input` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `request user input` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `request user input` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `request user input`:
...
Use a third `request user input`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `request user input`:
