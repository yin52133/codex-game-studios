...
- Source argument hint: `[sprint | feature: system-name] [--review full|lean|solo]`.
...
Spawn `qa-lead` via Codex sub-agent delegation to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `request user input`:
...
Spawn `qa-tester` via Codex sub-agent delegation for each story (run in parallel where possible), providing:
- The story file path
...
Use `request user input` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `request user input` for each:
...
After each FAIL result: use `request user input` to collect the failure description, then spawn `qa-tester` via Codex sub-agent delegation to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via Codex sub-agent delegation to produce the sign-off report using all results from Phases 4–6.
