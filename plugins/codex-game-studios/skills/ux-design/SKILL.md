---
name: ux-design
description: "Guided, section-by-section UX spec authoring for a screen, flow, or HUD. Reads game concept, player journey, and relevant GDDs to provide context-aware design guidance. Produces ux-spec.md (per screen/flow) or hud-design.md using the studio templates. (source alias: /ux-design)."
---
...
- Source workflow alias: `/ux-design`.
...
- Source argument hint: `[screen/flow name] or 'hud' or 'patterns'`.
...
**If no argument is provided**, do not fail — ask instead. Use `request user input`:
...
> "Player journey map not yet created. Template available at `references/templates/player-journey.md`. Run `/ux-design` Phase 2b or create it manually to establish player context for this screen."
...
Read `references/docs/technical-preferences.md` and extract the `## Input & Platform`
...
- Codex sub-agent delegation: Designing [screen/flow name] UX spec
...
2. **Questions**: Ask what is needed to draft this section. Use `request user input`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `request user input` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `request user input`:
...
7. **Write**: Use `request user input`: "May I write the [section name] section to `[filepath]`?"
...
- Use `request user input` to capture the choice:
...
- Use `request user input`: "Want an ASCII wireframe as part of this spec?"
...
Use `request user input` to confirm:
...
Use `request user input` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `request user input`:
...
- Codex sub-agent delegation: [screen-name] UX spec
...
When delegating to another agent via the Codex sub-agent delegation:
