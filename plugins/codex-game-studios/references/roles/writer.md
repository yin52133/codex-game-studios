...
Use the `request user input` tool for implementation choices and next-step decisions.
Follow the **Explain -> Capture** pattern: explain options in conversation, then
call `request user input` with concise labels. Batch up to 4 questions in one call.
