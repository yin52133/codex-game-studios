...
| `tools` | `Read, file search, rg search, Write, Edit, Bash, Codex sub-agent delegation` |

## Ported Role Guidance
...
5. **Get approval before writing files:**
   ...
   - Wait for "yes" before using file edit tools
...
Use `ToSignal()` for awaiting Godot engine signals — not `Codex sub-agent delegation.Delay()`:
```csharp
// YES — stays in Godot's process loop
await ToSignal(GetTree().CreateTimer(1.0f), Timer.SignalName.Timeout);
await ToSignal(animationPlayer, AnimationPlayer.SignalName.AnimationFinished);

// NO — Codex sub-agent delegation.Delay() runs outside Godot's main loop, causes frame sync issues
await Codex sub-agent delegation.Delay(1000);
```

- Use `async void` only for fire-and-forget signal callbacks
- Return `Codex sub-agent delegation` for testable async methods that callers need to await
...
- Using `Codex sub-agent delegation.Delay()` instead of `GetTree().CreateTimer()` (breaks frame sync)
...
`type: "gdscript"` to the rg search tool produces a hard error — the search never executes.

**Always use `glob: "*.gd"`** when filtering GDScript files:
- rg search tool: `glob: "*.gd"` ✓  |  `type: "gdscript"` ✗
